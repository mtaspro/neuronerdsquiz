...
import Modal from 'react-modal'; // Added for modals
...
export default function AdminDashboard() {
  ...
  const [questionsPerPage, setQuestionsPerPage] = useState(50); // Now configurable
  ...
  const handleUpdateQuizConfig = useCallback(async (chapterId, examQuestions, battleQuestions) => {
    ...
    try {
      const response = await createApiCall(`/api/admin/quiz-configs/${chapterId}`, {
        method: 'PUT',
        data: { examQuestions: 0, battleQuestions: parseInt(battleQuestions) || 0 },
      });
      ...
      setError(err.response?.data?.error || 'Failed to update battle config');
