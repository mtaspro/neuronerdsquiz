...
// Extract questions from URL (for mobile parser)
router.post('/extract-questions', sessionMiddleware, requireAdmin, async (req, res) => {
  try {
    const { url } = req.body;
    ...
    res.status(500).json({ success: false, error: 'Failed to extract content from URL' });
  }
});

// Add parsed questions to database (for mobile parser)
router.post('/add-parsed-questions', sessionMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    const savedQuestions = await Quiz.insertMany(questionsToAdd);
    res.json({ success: true, count: savedQuestions.length });
    
  } catch (error) {
    console.error('Error adding parsed questions:', error.message);
    res.status(500).json({ error: 'Failed to add questions to database' });
  }
});
